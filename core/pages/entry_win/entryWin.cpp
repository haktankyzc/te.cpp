...
void EntryWin::openEntryWin() {
  entryWin = newwin(Style::instance().term_rows(),
  ...
  int x = (Style::instance().term_cols() - Style::instance().logo_width()) /
          2; // Logo'yu ortaliyozs
...
void EntryWin::drawOptions(int highlighted) {
  ...
  int start_y =
      Style::instance().logo_height() + (Style::instance().term_cols() / 7) + 5;
